...
import { refreshToken } from '../services/sallaService';
import { TOKEN_REFRESH_CRON, TOKEN_REFRESH_BUFFER_HOURS } from '../lib/constants';
...
async function runTokenRefresh(): Promise<void> {
  ...
  const bufferMs = TOKEN_REFRESH_BUFFER_HOURS * 60 * 60 * 1000;
  ...
  try {
    ...
    for (const merchant of merchants) {
      try {
        await refreshToken(merchant.id, merchant.refresh_token);
        successCount++;
      } catch {
        failureCount++;
      }
...
export function startTokenRefreshJob(): void {
  ...
  console.log('[tokenRefresh] Cron job scheduled:', TOKEN_REFRESH_CRON);
}
