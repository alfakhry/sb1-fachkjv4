...
const inputClass = "rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 focus:outline-none transition";
    ...
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
      ...
      {open && (
        <div className="px-6 pb-6 border-t border-slate-100">
          <div className="space-y-3 mt-4">
            ...
            {filters.map((f, i) => (
              <div key={i} className="flex gap-2 items-center">
                <select
                  value={f.field}
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => updateRow(i, { field: e.target.value })}
                ...
                <select
                  value={f.operator}
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => updateRow(i, { operator: e.target.value })}
                ...
                <input
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) => updateRow(i, { value: e.target.value })}
                  placeholder="القيمة"
                  className={`w-28 ${inputClass} placeholder-slate-400`}
                />
                ...
                {f.operator === 'between' && (
                  <input
                    ...
                    onChange={(e: ChangeEvent<HTMLInputElement>) => updateRow(i, { value2: e.target.value })}
                    placeholder="حتى"
                    className={`w-20 ${inputClass} placeholder-slate-400`}
                  />
                )}
          ...
          <button
            onClick={addRow}
            className="mt-4 px-4 py-2 rounded-lg border border-dashed text-sm font-medium transition-colors"
