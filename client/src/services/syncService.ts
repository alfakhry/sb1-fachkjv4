...
import { SALLA_API_BASE, SYNC_PAGE_SIZE, SYNC_PAGE_DELAY_MS, PAYMENT_METHOD_COD } from '../lib/constants';
...
interface SallaOrderAmount {
  amount: number;
}

interface SallaOrderAmounts {
  total: SallaOrderAmount;
  sub_total: SallaOrderAmount;
  shipping?: SallaOrderAmount;
  tax?: SallaOrderAmount;
}
...
interface SallaOrder {
  ...
  shipping?: { city?: string; address?: string; tracking_number?: string };
  urls?: { customer?: string; rating?: string };
  customer: { id: string };
...
): { segment: string; segment_display: string } {
  if (totalOrders === 0) {
 ...
 */
export async function syncAllCustomers(merchant: MerchantForSync): Promise<number> {
  let page = 1;
  let lastPage = 1;
  let totalSynced = 0;

  do {
    const response = await axios.get<{ data: SallaCustomer[]; pagination: SallaPagination }>(
      `${SALLA_API_BASE}/customers`,
      {
        headers: { Authorization: `Bearer ${merchant.access_token}` },
        params: { per_page: SYNC_PAGE_SIZE, page },
      }
    );
 ...
 */
export async function syncAllOrders(merchant: MerchantForSync): Promise<number> {
  let page = 1;
  let lastPage = 1;
  let totalSynced = 0;

  do {
    const response = await axios.get<{ data: SallaOrder[]; pagination: SallaPagination }>(
    ...
    for (const order of orders) {
      ...
      await prisma.order_items.deleteMany({ where: { order_id: upsertedOrder.id } });
 ...
 */
export async function buildAllSmartProfiles(merchantId: number): Promise<void> {
 ...
 */
async function fetchSallaCategories(accessToken: string): Promise<string[]> {
  try {
    const response = await axios.get<{ data: SallaCategory[] }>(
      `${SALLA_API_BASE}/categories`,
...
): Promise<{ points: number; expiry: Date | null; found: boolean }> {
  try {
    const response = await axios.get<{ data: SallaLoyaltyPoint[] }>(
      `${SALLA_API_BASE}/customers/loyalty/points`,
      {
        headers: { Authorization: `Bearer ${accessToken}` },
    ...
    const activePoints = points.filter(
      (p) => p.status === 'مؤكدة' && (!p.expiry_date || new Date(p.expiry_date) > new Date())
    );
 ...
 */
export async function buildSmartProfile(merchantId: number, customerId: number): Promise<void> {
  ...
  const highestOrderValue = orders.reduce(
    (max, o) => Math.max(max, Number(o.total_amount ?? 0)),
    0
  );

  const orderDates = orders
    .filter((o) => o.salla_created_at !== null)
    .map((o) => o.salla_created_at as Date);
  ...
  // ── PAYMENT ANALYSIS ──
  const codOrdersCount = orders.filter((o) => o.payment_method === PAYMENT_METHOD_COD).length;
  const prepaidOrdersCount = orders.filter((o) => o.payment_method !== PAYMENT_METHOD_COD).length;
  let paymentPreference = 'mixed';
  if (codOrdersCount > prepaidOrdersCount) paymentPreference = 'cod';
  else if (prepaidOrdersCount > codOrdersCount) paymentPreference = 'prepaid';
  ...
  const repeatProducts: Record<string, { name: string; count: number }> = {};
  ...
  const mostReorderedProduct =
    Object.entries(repeatProducts).sort(([, a], [, b]) => b.count - a.count)[0]?.[0] ?? null;
  const totalUniqueProducts = Object.keys(productCounts).length;
  const reorderRate =
    totalUniqueProducts > 0 ? Object.keys(repeatProducts).length / totalUniqueProducts : 0;
  ...
  let neverBoughtCategories: string[] = [];
  ...
  const whatsappResponseRate =
    totalOrders > 0 ? (ordersFromWhatsapp / totalOrders) * 100 : 0;

  // ── TIME INTELLIGENCE ──
  const orderHours = orderDates.map((d) => d.getHours());
  const preferredPurchaseHour = mode(orderHours);

  let preferredPurchasePeriod: string | null = null;
  if (preferredPurchaseHour !== null) {
    if (preferredPurchaseHour >= 0 && preferredPurchaseHour <= 5) preferredPurchasePeriod = 'night';
    else if (preferredPurchaseHour >= 6 && preferredPurchaseHour <= 11) preferredPurchasePeriod = 'morning';
    else if (preferredPurchaseHour >= 12 && preferredPurchaseHour <= 17) preferredPurchasePeriod = 'afternoon';
    else preferredPurchasePeriod = 'evening';
  }
  ...
  const nextPurchasePredictedDate =
    avgDaysBetweenOrders > 0 && lastOrderDate
  ...
  const sortedOrdersByDate = [...orders].sort(
    (a, b) => (b.salla_created_at?.getTime() ?? 0) - (a.salla_created_at?.getTime() ?? 0)
  );
  const last3Orders = sortedOrdersByDate.slice(0, 3);
  const last3OrdersAvg =
  ...
  if (totalOrders >= 3) {
    if (last3OrdersAvg > avgOrderValue * 1.1) orderValueTrend = 'increasing';
    else if (last3OrdersAvg < avgOrderValue * 0.9) orderValueTrend = 'decreasing';
  }
  ...
  const freeShippingOrdersCount = orders.filter(
    (o) => Number(o.shipping_amount ?? 0) === 0
  ).length;

  // ── LOYALTY & WALLET ──
  let loyaltyPoints = 0;
  let loyaltyPointsExpiry: Date | null = null;
  ...
  await prisma.smart_profiles.upsert({
